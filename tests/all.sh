...
goal app create --creator $CREATOR --approval-prog $SYSTEM_APPROVAL_FILE --clear-prog $SYSTEM_CLEAR_FILE --global-byteslices 0 --global-ints 0 --local-byteslices 16 --local-ints 0 --on-completion OptIn -w $WALLET

// opt-in
goal app optin --app-id=$APP_ID --from $USER -w $WALLET

// set key valvue
goal app call --app-id=$APP_ID --from $USER --app-arg="str:$KEY" --app-arg="str:$VALUE" -w $WALLET
